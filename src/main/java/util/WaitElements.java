...
public class WaitElements {
    ...
    /**
     * Variable responsible for the timeout duration for object visibility
     */
    private static final Duration OBJECT_TIMEOUT = Duration.ofSeconds(30);
    ...
    public boolean visibilityElement(String by, String localizador) {
        WebDriverWait wait = new WebDriverWait(driver.getDriver(), OBJECT_TIMEOUT);
        try {
            wait.until(ExpectedConditions.visibilityOfElementLocated(UIElement.toBy(by, localizador)));
            return true;
        } catch (Exception e) {
            logger.error("Wait time expired for object '{}' ({})", localizador, by);
            return false;
        }
    }
